...
// Tint effect shader for spotlight overlay background.
...
const getShaderSource = (extensionDir) => {
    const SHADER_PATH = GLib.build_filenamev([extensionDir, 'spotlight', 'effects', 'tint_effect.glsl']);
    try {
        return Shell.get_file_contents_utf8_sync(SHADER_PATH);
    } catch (e) {
        console.error(`[mactop] Error loading shader from ${SHADER_PATH}: ${e}`);
        return null;
    }
};
...
export const TintEffect = GObject.registerClass(
    {},
    class MacTopTintEffect extends Clutter.ShaderEffect {
        _init(params) {
            this._red = null;
            this._green = null;
            this._blue = null;
        ...
        set blend(value) {
            if (value > 0.5) value = Math.max(0.5, value * 0.75);
